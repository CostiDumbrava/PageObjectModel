...
public class XpathExample2 extends BaseTest {
	...
	public void xpathExampleTest() throws InterruptedException {
		...
		/*
		 *  descendant
		 * //ul/descendant::input[@id='log']    <=>  //ul//input[@id='log']
		 */
		...
		//following-sibling --> merge in jos in DOM ;
		...
		Thread.sleep(3000);
		//precedind-sibling  --> merge in sus in DOM
		...
		//following =  cand folosim doar following sare mai jos indiferent de ierarhie, nu mai tine cont de relatiile dintre parinti si copii
		WebElement forgotPass = driver.findElement(By.xpath("//a[@href='tel:123-456-7890']/following::a[@class='forgot_password']"));
		jse.executeScript("arguments[0].setAttribute"
				+ "('style', 'background:yellow; border:4px solid red;')", forgotPass);
		
		//preceding = cand folosim doar preceding merge in sus indiferent de ierarhie, nu mai tine cont de relatiile dintre parinti si copii
		WebElement loginBtn = driver.findElement(By.xpath("//div[@class='top_panel_middle']/preceding::input[@class='submit_button']"));
		jse.executeScript("arguments[0].setAttribute"
